...
const ElegantCheckmark: React.FC<{ size: number; delay: number }> = ({
  size,
  delay,
}) => {
  const frame = useCurrentFrame();
  ...
  const strokeProgress = interpolate(progress, [0.3, 1], [0, 1], {
  ...
  return (
    ...
    >
      {words.map((word, i) => {
        ...
        const blur = interpolate(wordProgress, [0, 0.5], [8, 0], {
          extrapolateRight: "clamp",
        });
  ...
  return (
    ...
    >
      {pills.map((pill, i) => {
        ...
        // Glow pulse
        const glow =
          hoverFrame > 0 ? Math.sin(hoverFrame * 0.1 + i) * 0.3 + 0.7 : 0;
